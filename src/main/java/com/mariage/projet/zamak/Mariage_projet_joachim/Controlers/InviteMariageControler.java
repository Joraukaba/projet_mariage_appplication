...
public class InviteMariageControler {
    ...
    @GetMapping("/invitepreg/{id-prog}")
    public ResponseEntity<List<InviteMariageDto>>findAllByid(@PathVariable("id-prog") Integer id){
        return ResponseEntity.ok(services.findAllById(id));
    }
